...
use actix_web::{delete, dev::ConnectionInfo, get, post, web, HttpResponse, Scope};
...
use mitra_models::{
    ...
    posts::queries::{
        create_post, delete_post, find_reposts_by_user, get_post_by_id, get_thread,
        set_post_ipfs_cid,
    },
...
use crate::activitypub::{
    builders::{
        announce::prepare_announce, create_note::prepare_create_note,
        delete_note::prepare_delete_note, like::prepare_like, undo_announce::prepare_undo_announce,
...
use crate::validators::posts::{
    clean_content, ATTACHMENT_LIMIT, EMOJI_LIMIT, LINK_LIMIT, MENTION_LIMIT,
};
...
async fn create_status(
    ...
    let PostContent {
        ...
        linked,
        emojis,
    } = parse_microsyntaxes(db_client, &instance, content).await?;
    // Clean content
    ...
    if visibility == Visibility::Subscribers {
        ...
        // on other instances.
        let subscribers = get_subscribers(db_client, &current_user.id)
            .await?
            .into_iter()
...
async fn preview_status(
    ...
    content = clean_content(&content)?;
    // Return preview
    let preview = StatusPreview::new(&instance.url(), content, emojis);
...
async fn favourite(
    ...
    let maybe_reaction_created =
        match create_reaction(db_client, &current_user.id, &status_id, None).await {
            Ok(reaction) => {
...
async fn reblog(
    ...
    // Federate
    prepare_announce(db_client, &config.instance(), &current_user, &repost)
        .await?
        .enqueue(db_client)
        .await?;
...
async fn unreblog(
    ...
    let repost_id = reposts
        .first()
        .ok_or(MastodonError::NotFoundError("post"))?;
...
async fn make_permanent(
    ...
    for attachment in post.attachments.iter_mut() {
        ...
        let image_data = std::fs::read(image_path).map_err(|_| MastodonError::InternalError)?;
        let image_cid = ipfs_store::add(ipfs_api_url, image_data)
            .await
    ...
    let post_metadata_json = serde_json::to_string(&post_metadata)
        ...
        .as_bytes()
        .to_vec();
    let post_metadata_cid = ipfs_store::add(ipfs_api_url, post_metadata_json)
        .await
        .map_err(|_| MastodonError::InternalError)?;
